...
public class IqcModelProfileController {
    ...
    @GetMapping @ResourcePermission(code="iqc:model:view", name="查看模型配置", type="iqc", description="查询 Agent 模型配置") @Audit(operationType=OperationType.QUERY, description="查询 IQC 模型配置", module="IQC_MODEL")
    ...
    @PostMapping("/{id}/test") @ResourcePermission(code="iqc:model:test", name="测试模型连接", type="iqc", description="测试 Agent 模型配置连通性") @Audit(operationType=OperationType.QUERY, description="测试 IQC 模型连接", module="IQC_MODEL", request=false)
